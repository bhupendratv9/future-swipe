...
import { type GuestFormValues, guestSchema } from "@/schemas/profile-schema.ts";
import Header from "@/components/header/Header.tsx";
import BackButton from "@/components/common/BackButton.tsx";
import { Vstack } from "@/components/layout/Vstack.tsx";
import { Container } from "@/components/layout/Container.tsx";
import { Field, Form, Radio, RadioGroup } from "@base-ui/react";
...
export const Route = createFileRoute("/(auth)/_auth/guest-signup")({
...
function RouteComponent() {
  ...
  const genderOptions = [
    { value: "Male", label: pageContent?.personalInfo?.fields?.gender?.type?.male },
    { value: "Female", label: pageContent?.personalInfo?.fields?.gender?.type?.female },
  ] ;
  ...
  const decryptedPathname = encryptionUtils.decrypt(redirect as string);

  const { data: profile } = useGetProfileQuery();

  // Only redirect if profile exists AND user is NOT a guest
  // (guests should stay on this page until they submit the form)
  if (profile && profile?.data?.user?.is_guest !== 1) {
    router.navigate({ to: decryptedPathname || "/dashboard" });
  }

  const guestMutation = useGuestSignupMutation();
  ...
  const form = useForm({
    ...
    onSubmit: ({ value }) => {
      ...
      guestMutation.mutate(formData, {
        onSuccess: async () => {
          form.reset();

          // Determine target path
          const targetPath = decryptedPathname || "/result-unlock";

          if(targetPath === "/result-unlock"){
            toast.success("Results unlocked (Create an account to save progress)", {
              action:{
                label: "Login",
                onClick:() => {
                  router.navigate({to: "/sign-in", search:{redirect: redirect}})
                }
              }
            })
          } else {
            toast.success("Playing as guest (Create an account to save progress)",{
              action:{
                label: "Login",
                onClick:() => {
                  router.navigate({to: "/sign-in", search:{redirect: redirect}})
          ...
          // Hard reload to clear all stale cache and use fresh token
          window.location.href = import.meta.env.BASE_URL + "result-unlock";
        },
        onError: (error) => {
        ...
        <Container className="max-w-120">
          <div className="max-w-91 mx-auto">
            <Form
              ...
              className={"space-y-10 min-h-[calc(100vh-20vh)] flex items-center justify-center font-montserrat"}
            >
              <motion.div
                  ...
                  <div className="space-y-10">
                    <div className="space-y-2">
                      <p className="font-unbounded text-xl leading-8">
                       {pageContent?.personalInfo?.title}
                      </p>
                      <p className="font-light leading-6.5">
                       {pageContent?.personalInfo?.subTitle}
                      </p>
                    </div>
                                ...
                                >
                                  {field.state.value ? (
                                    <div className="flex bg-white/10 px-5 py-3.75 items-center text-[13px] rounded-full gap-2.5 font-semibold text-[#8D8D8D]">
                                      <ReloadIconSvg />
                                      <span>{pageContent?.personalInfo?.fields?.upload?.change}</span>
                                    </div>
                                  ) : (
                                    <div className="flex bg-white/10 px-5 py-3.75 items-center text-[13px] text-secondary rounded-full gap-2.5 font-semibold">
                                      ...
                                      <span>{pageContent?.personalInfo?.fields?.upload?.placeholder}</span>
                                    </div>
                                  )}
                                </label>
                                <>
                          ...
                          return (
                            ...
                            >
                              ...
                              <Field.Control
                                ...
                                placeholder= {pageContent?.personalInfo?.fields?.name?.placeholder}
                          ...
                          return (
                            ...
                            >
                              ...
                              <Field.Control
                                ...
                                placeholder=  {pageContent?.personalInfo?.fields?.age?.placeholder}
                                value={
                                  field.state.value === 0
                                    ? ""
                      ...
                      {/* <form.Field name="qualification">
                        {(field) => {
                          ...
                          return (
                            ...
                            >
                              <Field.Label className="font-medium text-white/30 text-sm">
                                 {t("personalInfo.fields.qualification.label")}
                              </Field.Label>
                              <Select.Root
                                value={field.state.value as string}
                                onValueChange={(value) =>
                              ...
                              >
                                <Select.Trigger className="flex items-center justify-between bg-white/20 rounded-[10px] px-3.5 h-12.5 text-[15px] w-full cursor-pointer">
                                  <Select.Value
                                    placeholder= {t("personalInfo.fields.qualification.placeholder")}
